...
public class AnnotationImpl implements InvocationHandler {
	...
	static {
		...
		try {
			...
			JDK_ANNOTATION_TYPE_METHOD = clazz.getMethod("annotationType",
					(Class[]) null);
		...
		if (annotationType == null) {
			...
			try {
				...
				NoClassDefFoundError error = new NoClassDefFoundError(
						"Error loading annotation class: " + typeName);
		...
		if (cachedHashCode == Integer.MIN_VALUE) {
			...
			for (int i = 0; i < methods.length; ++i) {
				...
				try {
					...
					throw new RuntimeException("Error retrieving value " + name
							+ " for annotation " + annotation.getTypeName(), e);
				}
	...
	private boolean checkEquals(Object obj) throws Exception {
		...
		Class otherAnnotationType = (Class) JDK_ANNOTATION_TYPE_METHOD.invoke(
				obj, (Object[]) null);
