...
public class SignatureAttribute extends AttributeInfo {
	...
	public static class ClassSignature {
		 ...
		 *            the interface types.
		 */
		public ClassSignature(TypeParameter[] params, ClassType superClass,
				ClassType[] interfaces) {
			this.params = params == null ? new TypeParameter[0] : params;
			this.superClass = superClass == null ? ClassType.OBJECT
					: superClass;
			this.interfaces = interfaces == null ? new ClassType[0]
					: interfaces;
		}
	...
	public static class MethodSignature {
		 ...
		 *            exception types.
		 */
		public MethodSignature(TypeParameter[] tp, Type[] params, Type ret,
				ObjectType[] ex) {
	...
	public static class TypeParameter {
		 ...
		 */
		public TypeParameter(String name, ObjectType superClass,
				ObjectType[] superInterfaces) {
			this.name = name;
	...
	public static class ClassType extends ObjectType {
		...
		static ClassType make(String s, int b, int e, TypeArgument[] targs,
				ClassType parent) {
			if (parent == null)
	...
	public static class NestedClassType extends ClassType {
		...
		NestedClassType(String s, int b, int e, TypeArgument[] targs,
				ClassType p) {
			super(s, b, e, targs);
			parent = p;
		}
		 ...
		 */
		public NestedClassType(ClassType parent, String className,
				TypeArgument[] args) {
	...
	private static ClassSignature parseSig(String sig) throws BadBytecode,
		...
		ClassType[] ifs = (ClassType[]) ifArray.toArray(new ClassType[ifArray
				.size()]);
	...
	private static MethodSignature parseMethodSig(String sig)
		...
		ObjectType[] ex = (ObjectType[]) exceptions
				.toArray(new ObjectType[exceptions.size()]);
	...
	private static TypeParameter[] parseTypeParams(String sig, Cursor cur)
		...
		if (sig.charAt(cur.position) == '<') {
			...
			while (sig.charAt(cur.position) != '>') {
				...
				TypeParameter p = new TypeParameter(sig, nameBegin, nameEnd,
						classBound,
						(ObjectType[]) ifBound.toArray(new ObjectType[ifBound
								.size()]));
				typeParam.add(p);
			}
		...
		return (TypeParameter[]) typeParam.toArray(new TypeParameter[typeParam
				.size()]);
	}

	private static ObjectType parseObjectType(String sig, Cursor c,
	...
	private static ClassType parseClassType2(String sig, Cursor c,
			ClassType parent) throws BadBytecode {
	...
	private static TypeArgument[] parseTypeArgs(String sig, Cursor c)
			throws BadBytecode {
	...
	private static ObjectType parseArray(String sig, Cursor c)
			throws BadBytecode {
		int dim = 1;
