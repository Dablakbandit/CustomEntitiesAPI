...
public abstract class TypeData {
	 ...
	 */
	private static void setType(TypeData td, String className, ClassPool cp)
			throws BadBytecode {
	...
	public static class TypeVar extends AbsTypeVar {
		protected ArrayList lowers; // lower bounds of this type.
									// ArrayList<TypeData>
		...
		// depth-first serach
		public int dfs(ArrayList preOrder, int index, ClassPool cp)
				throws NotFoundException {
			if (visited > 0)
		...
		private void fixTypes(ArrayList scc, ClassPool cp)
				throws NotFoundException {
		...
		private String fixTypes2(ArrayList scc, HashSet lowersSet, ClassPool cp)
				throws NotFoundException {
		...
		private static boolean isObjectArray(CtClass cc)
				throws NotFoundException {
			return cc.isArray()
					&& cc.getComponentType().getSuperclass() == null;
		}

		private CtClass fixByUppers(ArrayList users, ClassPool cp,
				HashSet visited, CtClass type) throws NotFoundException {
			if (users == null)
				return type;
	...
	public static CtClass commonSuperClassEx(CtClass one, CtClass two)
		...
		if (one == two)
			...
			if (element == ele1)
				...
				return one.getClassPool().get(
						element == null ? "java.lang.Object" : element
								.getName() + "[]");
	 ...
	 * method is a copy from ja.bytecode.analysis.Type.
	 */
	...
	static boolean eq(CtClass one, CtClass two) {
		return one == two
				|| (one != null && two != null && one.getName().equals(
						two.getName()));
	}
