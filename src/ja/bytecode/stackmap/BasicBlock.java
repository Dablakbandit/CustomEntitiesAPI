...
public class BasicBlock {
	...
	protected void toString2(StringBuffer sbuf) {
		sbuf.append("pos=").append(position).append(", len=").append(length)
				.append(", in=").append(incoming).append(", exit{");
		if (exit != null) {
	...
	public static class Maker {
		...
		public BasicBlock[] make(CodeIterator ci, int begin, int end,
				ExceptionTable et) throws BadBytecode {
		...
		private Mark makeMark0(HashMap table, int pos, boolean isBlockBegin,
				boolean isTarget) {
		...
		private HashMap makeMarks(CodeIterator ci, int begin, int end,
			...
			while (ci.hasNext()) {
				...
				if ((Opcode.IFEQ <= op && op <= Opcode.IF_ACMPNE)
					...
					makeMark(marks, index, makeArray(to.block, next.block), 3,
							false);
				} else if (Opcode.GOTO <= op && op <= Opcode.LOOKUPSWITCH)
					switch (op) {
					case Opcode.GOTO:
						makeGoto(marks, index, index + ci.s16bitAt(index + 1),
								3);
						break;
					case Opcode.JSR:
				...
				else if ((Opcode.IRETURN <= op && op <= Opcode.RETURN)
						|| op == Opcode.ATHROW)
		...
		private BasicBlock[] makeBlocks(HashMap markTable) {
			Mark[] marks = (Mark[]) markTable.values().toArray(
					new Mark[markTable.size()]);
			java.util.Arrays.sort(marks);
			ArrayList blocks = new ArrayList();
			int i = 0;
			BasicBlock prev;
			if (marks.length > 0 && marks[0].position == 0
					&& marks[0].block != null)
