...
public class MapMaker extends Tracer {
	 ...
	 * useMain2 = args[0].equals("0"); if (useMain2 && args.length > 1) {
	 * main2(args); return; }
	 * 
	 * for (int i = 0; i < args.length; i++) main1(args[i]); }
	 * 
	 * public static void main1(String className) throws Exception { ClassPool
	 * cp = ClassPool.getDefault(); //ja.CtClass cc = cp.get(className);
	 * ja.CtClass cc = cp.makeClass(new java.io.FileInputStream(className));
	 * System.out.println(className); ClassFile cf = cc.getClassFile();
	 * java.util.List minfos = cf.getMethods(); for (int i = 0; i <
	 * minfos.size(); i++) { MethodInfo minfo = (MethodInfo)minfos.get(i);
	 ...
	 * ClassPool.getDefault(); //ja.CtClass cc = cp.get(args[1]); ja.CtClass cc
	 * = cp.makeClass(new java.io.FileInputStream(args[1])); MethodInfo minfo;
	 * if (args[2].equals("_init_")) minfo =
	 ...
	 * 
	 * TypedBlock[] blocks = TypedBlock.makeBlocks(minfo, ca, false); MapMaker
	 * mm = new MapMaker(cp, minfo, ca); mm.make(blocks, ca.getCode()); for (int
	 * i = 0; i < blocks.length; i++) System.out.println(blocks[i]); }
	 */

	/**
	 * Computes the stack map table of the given method and returns it. It
	 * returns null if the given method does not have to have a stack map table
	 * or it includes JSR.
	 */
	...
	/**
	 * Computes the stack map table for J2ME. It returns null if the given
	...
	public MapMaker(ClassPool classes, MethodInfo minfo, CodeAttribute ca) {
		super(classes, minfo.getConstPool(), ca.getMaxStack(), ca
				.getMaxLocals(), TypedBlock.getRetType(minfo.getDescriptor()));
	}
	...
	private void traceException(byte[] code, TypedBlock.Catch handler)
		...
		while (handler != null) {
			...
			if (tb.alreadySet()) {
				mergeMap(tb, false);
				if (tb.stackTop < 1)
					throw new BadBytecode("bad catch clause: "
							+ handler.typeIndex);
	...
	private void recordStackMap0(TypedBlock target, int st,
			TypeData[] tStackTypes) throws BadBytecode {
		int n = localsTypes.length;
	...
	protected static int recordTypeData(int n, TypeData[] srcTypes,
			TypeData[] destTypes) {
		int k = -1;
		for (int i = 0; i < n; i++) {
	...
	protected static void copyTypeData(int n, TypeData[] srcTypes,
			TypeData[] destTypes) {
		for (int i = 0; i < n; i++)
			destTypes[i] = srcTypes[i];
	}

	private static TypeData validateTypeData(TypeData[] data, int length,
			int index) {
	 ...
	 * thrown within the handler itself. It is dead code. See
	 * ja.JvstTest4.testJIRA195().
	 */
	...
	/*
	 * This method first finds strongly connected components (SCCs) in a
	 * TypeData graph by Tarjan's algorithm. SCCs are TypeData nodes sharing the
	 * same type. Since SCCs are found in the topologically sorted order, their
	 * types are also fixed when they are found.
	 */
	private void fixTypes(byte[] code, TypedBlock[] blocks)
		...
		for (int i = 0; i < len; i++) {
			...
			if (block.alreadySet()) { // if block is not dead code
				...
				for (int j = 0; j < n; j++)
					index = block.localsTypes[j]
							.dfs(preOrder, index, classPool);
		...
		if (stackTop == 0) {
			if (diffL == 0) {
				...
				int[] tags = fillStackMap(bb.numLocals - prev.numLocals,
						prev.numLocals, data, bb.localsTypes);
	...
	private static boolean stackMapEq(TypeData[] oldTd, TypeData[] newTd,
			int len) {
		for (int i = 0; i < len; i++) {
	...
	private void writeStackFrame(StackMap.Writer writer, ConstPool cp,
			int offset, TypedBlock tb) {
