...
public class TypedBlock extends BasicBlock {
	...
	/**
	 * Divides the method body into basic blocks. The type information of the
	 * first block is initialized.
	 * 
	 * @param optmize
	 *            if it is true and the method does not include branches, this
	 *            method returns null.
	 */
	public static TypedBlock[] makeBlocks(MethodInfo minfo, CodeAttribute ca,
			boolean optimize) throws BadBytecode {
		...
		blocks[0].initFirstBlock(ca.getMaxStack(), ca.getMaxLocals(),
				pool.getClassName(), minfo.getDescriptor(), isStatic,
				minfo.isConstructor());
		return blocks;
	}
	 ...
	 *            <code>ja.bytecode.stackmap.BasicBlock</code>.
	 * @param methodDesc
	 *            method descriptor.
	 * @param isStatic
	...
	private static int descToTag(String desc, int i, int n, TypeData[] types)
		...
		if (c == 'L') {
			...
			if (arrayDim > 0)
				...
				types[n] = new TypeData.ClassName(desc.substring(i0 + 1,
						++i2 - 1).replace('/', '.'));
			return i2;
		} else if (arrayDim > 0) {
