...
public abstract class Tracer implements TypeTag {
	...
	protected int doOpcode(int pos, byte[] code) throws BadBytecode {
		try {
			...
			throw new BadBytecode(
					"inconsistent stack height " + e.getMessage(), e);
		}
	}
	 ...
	 */
	protected void visitTableSwitch(int pos, byte[] code, int n, int offsetPos,
			int defaultOffset) throws BadBytecode {
	}
	 ...
	 *            the offset to the default branch target.
	 */
	protected void visitLookupSwitch(int pos, byte[] code, int n, int pairsPos,
			int defaultOffset) throws BadBytecode {
	}
	...
	private int doOpcode54_95(int pos, byte[] code, int op) throws BadBytecode {
		...
		case Opcode.AASTORE:
			TypeData.ArrayElement.aastore(stackTypes[stackTop - 3],
					stackTypes[stackTop - 1], classPool);
			stackTop -= 3;
			break;
		case Opcode.BASTORE:
		case Opcode.CASTORE:
	...
	private int doOpcode148_201(int pos, byte[] code, int op)
			throws BadBytecode {
		switch (op) {
		case Opcode.LCMP:
		...
		case Opcode.TABLESWITCH: {
			...
			int n = high - low + 1;
			visitTableSwitch(pos, code, n, pos2 + 8,
					ByteArray.read32bit(code, pos2 - 4));
			return n * 4 + 16 - (pos & 3);
		}
		case Opcode.LOOKUPSWITCH: {
			...
			visitLookupSwitch(pos, code, n, pos2 + 4,
					ByteArray.read32bit(code, pos2 - 4));
			return n * 8 + 12 - (pos & 3);
		}
		case Opcode.IRETURN:
		...
		case Opcode.NEW: {
			...
			stackTypes[stackTop++] = new TypeData.UninitData(pos,
					cpool.getClassInfo(i));
			return 3;
		}
		...
		case Opcode.CHECKCAST: {
			// TypeData.setType(stackTypes[stackTop - 1], "java.lang.Object",
			// classPool);
			int i = ByteArray.readU16bit(code, pos + 1);
	...
	private int doPutField(int pos, byte[] code, boolean notStatic)
			throws BadBytecode {
	...
	private int doGetField(int pos, byte[] code, boolean notStatic)
			throws BadBytecode {
		int index = ByteArray.readU16bit(code, pos + 1);
		setFieldTarget(notStatic, index);
		String desc = cpool.getFieldrefType(index);
	...
	private void setFieldTarget(boolean notStatic, int index)
			throws BadBytecode {
		if (notStatic) {
	...
	private int doInvokeMethod(int pos, byte[] code, boolean notStatic)
			throws BadBytecode {
		int i = ByteArray.readU16bit(code, pos + 1);
		...
		if (notStatic) {
			...
			if (target instanceof TypeData.UninitTypeVar && target.isUninit())
				constructorCalled(target,
						((TypeData.UninitTypeVar) target).offset());
			else if (target instanceof TypeData.UninitData)
				constructorCalled(target,
						((TypeData.UninitData) target).offset());
	...
	private void pushMemberType(String descriptor) {
		...
		case '[':
			...
			break;
		case 'L':
			types[index] = new TypeData.ClassName(getFieldClassName(descriptor,
					top));
			break;
		case 'J':
			types[index] = LONG;
