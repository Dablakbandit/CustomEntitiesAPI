...
class CodeAnalyzer implements Opcode {
	...
	private boolean processBranch(int opcode, CodeIterator ci, int index,
			...
			throws BadBytecode {
		if ((IFEQ <= opcode && opcode <= IF_ACMPNE) || opcode == IFNULL
				|| opcode == IFNONNULL) {
			...
			case JSR_W:
				...
				checkTarget(index, target, codeLength, stack, stackDepth);
				/*
				 * It is unknown which RET comes back to this JSR. So we assume
				 * that if the stack depth at one JSR instruction is N, then it
				 * is also N at other JSRs and N - 1 at all RET instructions.
				 * Note that STACK_GROW[JSR] is 1 since it pushes a return
			...
			case TABLESWITCH:
				...
				if (opcode == LOOKUPSWITCH) {
					...
					for (int i = 0; i < npairs; ++i) {
						target = index + ci.s32bitAt(index2);
						checkTarget(index, target, codeLength, stack,
								stackDepth);
						index2 += 8;
					}
				} else {
	...
	private void checkTarget(int opIndex, int target, int codeLength,
			int[] stack, int stackDepth) throws BadBytecode {
		...
		if (d == 0)
			...
			throw new BadBytecode("verification error (" + stackDepth + "," + d
					+ ") at " + opIndex);
	}
