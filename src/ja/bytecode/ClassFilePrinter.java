...
/**
 * A utility class for priting the contents of a class file. It prints a
 * constant pool table, fields, and methods in a human readable representation.
 */
public class ClassFilePrinter {
	...
	public static void print(ClassFile cf, PrintWriter out) {
		 ...
		 * class.
		 */
		int mod = AccessFlag.toModifier(cf.getAccessFlags()
				& ~AccessFlag.SYNCHRONIZED);
		out.println("major: " + cf.major + ", minor: " + cf.minor
				+ " modifiers: " + Integer.toHexString(cf.getAccessFlags()));
		out.println(Modifier.toString(mod) + " class " + cf.getName()
				+ " extends " + cf.getSuperclass());
		...
		for (int i = 0; i < n; ++i) {
			...
			out.println(Modifier.toString(AccessFlag.toModifier(acc)) + " "
					+ finfo.getName() + "\t" + finfo.getDescriptor());
		...
		for (int i = 0; i < n; ++i) {
			...
			if (ai instanceof CodeAttribute) {
				...
				out.println("attribute: " + ai.getName() + ": "
						+ ai.getClass().getName());
				out.println("max stack " + ca.getMaxStack() + ", max locals "
						+ ca.getMaxLocals() + ", "
						+ ca.getExceptionTable().size() + " catch blocks");
				...
				try {
					...
					if (kind == 'c')
						...
						s = SignatureAttribute.toMethodSignature(sig)
								.toString();
					else
