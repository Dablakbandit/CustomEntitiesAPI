...
public class ControlFlow {
	...
	public Node[] postDominatorTree() {
		...
		for (int i = 0; i < size; i++)
			if (nodes[i].block.exits() == 0)
				counter = nodes[i].makeDepth1stTree(null, visited, counter,
						distance, access);
	...
	public static class Block extends BasicBlock {
		 ...
		 * attribute to a <code>Block</code> object. The ja library never
		 * accesses this field.
		 */
	...
	public static class Node {
		...
		public String toString() {
			...
			sbuf.append(", parent=");
			sbuf.append(parent == null ? "*" : Integer.toString(parent.block()
					.position()));
		 ...
		 */
		int makeDepth1stTree(Node caller, boolean[] visited, int counter,
				int[] distance, Access access) {
			...
			if (exits != null)
				for (int i = 0; i < exits.length; i++) {
					...
					counter = n.makeDepth1stTree(this, visited, counter,
							distance, access);
				}
		...
		boolean makeDominatorTree(boolean[] visited, int[] distance,
			...
			if (entrances != null)
				for (int i = 0; i < entrances.length; i++) {
					if (parent != null) {
						Node n = getAncestor(parent, access.node(entrances[i]),
								distance);
						if (n != parent) {
							parent = n;
							changed = true;
