...
public class Executor implements Opcode {
	 ...
	 */
	public void execute(MethodInfo method, int pos, CodeIterator iter,
			Frame frame, Subroutine subroutine) throws BadBytecode {
		this.lastPos = pos;
		...
		case POP:
			if (frame.pop() == Type.TOP)
				throw new BadBytecode(
						"POP can not be used with a category 2 value, pos = "
								+ pos);
			break;
		case POP2:
			frame.pop();
		...
		case DUP: {
			...
			if (type == Type.TOP)
				throw new BadBytecode(
						"DUP can not be used with a category 2 value, pos = "
								+ pos);
		...
		case SWAP: {
			...
			if (type1.getSize() == 2 || type2.getSize() == 2)
				throw new BadBytecode(
						"Swap can not be used with category 2 values, pos = "
								+ pos);
			frame.push(type1);
			frame.push(type2);
			break;
		...
		case JSR:
			...
			break;
		case RET:
			verifyAssignable(Type.RETURN_ADDRESS,
					frame.getLocal(iter.byteAt(pos + 1)));
			break;
		case TABLESWITCH:
		case LOOKUPSWITCH:
		case IRETURN:
		...
		case ARETURN:
			try {
				CtClass returnType = Descriptor.getReturnType(
						method.getDescriptor(), classPool);
		...
		case INVOKEDYNAMIC:
			...
			break;
		case NEW:
			frame.push(resolveClassInfo(constPool.getClassInfo(iter
					.u16bitAt(pos + 1))));
			break;
		case NEWARRAY:
		...
		case ARRAYLENGTH: {
			...
			if (!array.isArray() && array != Type.UNINIT)
				throw new BadBytecode("Array length passed a non-array [pos = "
						+ pos + "]: " + array);
			frame.push(Type.INTEGER);
			break;
		}
		...
		case CHECKCAST:
			verifyAssignable(Type.OBJECT, simplePop(frame));
			frame.push(typeFromDesc(constPool.getClassInfoByDescriptor(iter
					.u16bitAt(pos + 1))));
			break;
		case INSTANCEOF:
			verifyAssignable(Type.OBJECT, simplePop(frame));
	...
	private Type zeroExtend(Type type) {
		if (type == Type.SHORT || type == Type.BYTE || type == Type.CHAR
				|| type == Type.BOOLEAN)
			return Type.INTEGER;
	...
	private void evalArrayLoad(Type expectedComponent, Frame frame)
			throws BadBytecode {
		...
		if (component == null)
			throw new BadBytecode("Not an array! [pos = " + lastPos + "]: "
					+ component);
	...
	private void evalArrayStore(Type expectedComponent, Frame frame)
			throws BadBytecode {
	...
	private void evalGetField(int opcode, int index, Frame frame)
			throws BadBytecode {
		String desc = constPool.getFieldrefType(index);
		Type type = zeroExtend(typeFromDesc(desc));

		if (opcode == GETFIELD) {
			Type objectType = resolveClassInfo(constPool
	...
	private void evalInvokeIntfMethod(int opcode, int index, Frame frame)
			throws BadBytecode {
	...
	private void evalInvokeMethod(int opcode, int index, Frame frame)
			throws BadBytecode {
	...
	private void evalInvokeDynamic(int opcode, int index, Frame frame)
			throws BadBytecode {
	...
	private void evalLoad(Type expected, int index, Frame frame,
			Subroutine subroutine) throws BadBytecode {
	...
	private void evalNewArray(int pos, CodeIterator iter, Frame frame)
			throws BadBytecode {
		verifyAssignable(Type.INTEGER, simplePop(frame));
		Type type = null;
		...
		case T_DOUBLE:
			...
			break;
		default:
			throw new BadBytecode("Invalid array type [pos = " + pos + "]: "
					+ typeInfo);
	...
	private void evalNewObjectArray(int pos, CodeIterator iter, Frame frame)
			throws BadBytecode {
		// Convert to x[] format
		Type type = resolveClassInfo(constPool.getClassInfo(iter
				.u16bitAt(pos + 1)));
	...
	private void evalPutField(int opcode, int index, Frame frame)
			throws BadBytecode {
		String desc = constPool.getFieldrefType(index);
		Type type = zeroExtend(typeFromDesc(desc));
	...
	private void evalStore(Type expected, int index, Frame frame,
			Subroutine subroutine) throws BadBytecode {
	...
	private void evalWide(int pos, CodeIterator iter, Frame frame,
			Subroutine subroutine) throws BadBytecode {
		...
		case RET:
			...
			break;
		default:
			throw new BadBytecode("Invalid WIDE operand [pos = " + pos + "]: "
					+ opcode);
		}
	...
	private Type getType(String name) throws BadBytecode {
		try {
			...
			throw new BadBytecode("Could not find class [pos = " + lastPos
					+ "]: " + name);
		}
	}
	...
	private Type[] paramTypesFromDesc(String desc) throws BadBytecode {
		...
		try {
			...
			throw new BadBytecode("Could not find class in descriptor [pos = "
					+ lastPos + "]: " + e.getMessage());
		}

		if (classes == null)
			throw new BadBytecode(
					"Could not obtain parameters for descriptor [pos = "
							+ lastPos + "]: " + desc);
	...
	private Type returnTypeFromDesc(String desc) throws BadBytecode {
		...
		if (clazz == null)
			throw new BadBytecode(
					"Could not obtain return type for descriptor [pos = "
							+ lastPos + "]: " + desc);
	...
	private Type simplePeek(Frame frame) {
		Type type = frame.peek();
		return (type == Type.TOP) ? frame.getStack(frame.getTopIndex() - 1)
				: type;
	}
	...
	private Type resolveClassInfo(String info) throws BadBytecode {
		...
		if (clazz == null)
			throw new BadBytecode(
					"Could not obtain type for descriptor [pos = " + lastPos
							+ "]: " + info);
	...
	private void verifyAssignable(Type expected, Type type) throws BadBytecode {
		if (!expected.isAssignableFrom(type))
			throw new BadBytecode("Expected type: " + expected + " Got: "
					+ type + " [pos = " + lastPos + "]");
	}
}
