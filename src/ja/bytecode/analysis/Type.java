...
public class Type {
	 ...
	 */
	public int getSize() {
		return clazz == CtClass.doubleType || clazz == CtClass.longType
				|| this == TOP ? 2 : 1;
	}
	...
	public boolean isAssignableFrom(Type type) {
		...
		if ((type == UNINIT && isReference()) || this == UNINIT
				&& type.isReference())
			return true;
	...
	private Type mergeArray(Type type) {
		...
		if (eq(CLONEABLE.clazz, targetRoot.clazz)
				|| eq(SERIALIZABLE.clazz, targetRoot.clazz))
			return createArray(targetRoot, targetDims);

		return createArray(OBJECT, targetDims);
	}

	private static CtClass findCommonSuperClass(CtClass one, CtClass two)
