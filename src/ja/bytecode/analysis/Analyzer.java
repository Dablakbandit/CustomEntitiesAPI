...
public class Analyzer implements Opcode {
	...
	public Frame[] analyze(CtClass clazz, MethodInfo method) throws BadBytecode {
		...
		Executor executor = new Executor(clazz.getClassPool(),
				method.getConstPool());
	...
	private void analyzeNextEntry(MethodInfo method, CodeIterator iter,
		...
		if (opcode == TABLESWITCH) {
			...
			if (Util.isJsr(opcode)) {
				...
				mergeJsr(queue, frames[pos], subroutines[target], pos,
						lookAhead(iter, pos));
	...
	private ExceptionInfo[] buildExceptionInfo(MethodInfo method) {
		...
		for (int i = 0; i < table.size(); i++) {
			...
			Type type;
			try {
				type = index == 0 ? Type.THROWABLE : Type.get(classes
						.get(constPool.getClassInfo(index)));
			} catch (NotFoundException e) {
				throw new IllegalStateException(e.getMessage());
			}

			exceptions[i] = new ExceptionInfo(table.startPc(i), table.endPc(i),
					table.handlerPc(i), type);
		}
	...
	private Frame firstFrame(MethodInfo method, int maxLocals, int maxStack) {
		...
		CtClass[] parameters;
		try {
			parameters = Descriptor.getParameterTypes(method.getDescriptor(),
					clazz.getClassPool());
		} catch (NotFoundException e) {
	...
	private int getNext(CodeIterator iter, int of, int restore)
			throws BadBytecode {
		iter.move(of);
		iter.next();
	...
	private void mergeExceptionHandlers(IntQueue queue, MethodInfo method,
			int pos, Frame frame) {
	...
	private void mergeJsr(IntQueue queue, Frame frame, Subroutine sub, int pos,
	...
	private void mergeLookupSwitch(IntQueue queue, int pos, CodeIterator iter,
			Frame frame) throws BadBytecode {
		int index = (pos & ~3) + 4;
		// default
	...
	private void mergeRet(IntQueue queue, CodeIterator iter, int pos,
	...
	private void mergeTableSwitch(IntQueue queue, int pos, CodeIterator iter,
			Frame frame) throws BadBytecode {
