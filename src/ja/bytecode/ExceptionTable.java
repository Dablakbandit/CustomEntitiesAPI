...
public class ExceptionTable implements Cloneable {
	...
	/**
	 * Returns <code>exception_table_length</code>, which is the number of
	 * entries in the <code>exception_table[]</code>.
	 */
	public int size() {
		return entries.size();
	}
	 ...
	 * @param nth
	 *            the <i>n</i>-th (&gt;= 0).
	 * @return an index into the <code>constant_pool</code> table, or zero if
	 *         this exception handler is for all exceptions.
	 */
	...
	public void add(int index, ExceptionTable table, int offset) {
		int len = table.size();
		while (--len >= 0) {
			ExceptionTableEntry e = (ExceptionTableEntry) table.entries
					.get(len);
			add(index, e.startPc + offset, e.endPc + offset, e.handlerPc
					+ offset, e.catchType);
		}
	}
	...
	public void add(int index, int start, int end, int handler, int type) {
		if (start < end)
			entries.add(index, new ExceptionTableEntry(start, end, handler,
					type));
	}
	...
	/**
	 * Makes a copy of this <code>exception_table[]</code>. Class names are
	 * replaced according to the given <code>Map</code> object.
	 * 
	 * @param newCp
