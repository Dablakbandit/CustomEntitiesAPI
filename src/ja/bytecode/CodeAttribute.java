...
public class CodeAttribute extends AttributeInfo implements Opcode {
	...
	/**
	 * Returns the name of the class declaring the method including this code
	 * attribute.
	 */
	...
	/**
	 * Computes the maximum stack size and sets <code>max_stack</code> to the
	 * computed size.
	 * 
	 * @throws BadBytecode
	...
	/**
	 * Returns <code>attributes[]</code>. It returns a list of
	 * <code>AttributeInfo</code>. A new element can be added to the returned
	...
	/**
	 * Adds a stack map table. If another copy of stack map table is already
	 * contained, the old one is removed.
	 * 
	 * @param smt
	 *            the stack map table added to this code attribute. If it is
	 *            null, a new stack map is not added. Only the old stack map is
	 *            removed.
	 */
	...
	/**
	 * Adds a stack map table for J2ME (CLDC). If another copy of stack map
	 * table is already contained, the old one is removed.
	 * 
	 * @param sm
	 *            the stack map table added to this code attribute. If it is
	 *            null, a new stack map is not added. Only the old stack map is
	...
	private byte[] copyCode(ConstPool destCp, Map classnames,
			ExceptionTable etable, CodeAttribute destCa) throws BadBytecode {
	...
	private static LdcEntry copyCode(byte[] code, int beginPos, int endPos,
			ConstPool srcCp, byte[] newcode, ConstPool destCp, Map classnameMap)
			throws BadBytecode {
		int i2, index;
	...
	private static void copyConstPoolInfo(int i, byte[] code, ConstPool srcCp,
			byte[] newcode, ConstPool destCp, Map classnameMap) {
	...
	/**
	 * Changes the index numbers of the local variables to append a new
	 * parameter. This method does not update
	 * <code>LocalVariableAttribute</code>,
	 * <code>LocalVariableTypeAttribute</code>, <code>StackMapTable</code>, or
	 * <code>StackMap</code>. These attributes must be explicitly updated.
	 * 
	 * @param where
	...
	/**
	 * @param lessThan
	 *            If the index of the local variable is less than this value, it
	 *            does not change. Otherwise, the index is increased.
	 * @param delta
	 *            the indexes of the local variables are increased by this
	 *            value.
	 */
	private static void shiftIndex(CodeIterator ci, int lessThan, int delta)
		...
		if (opcode < ILOAD)
			...
			if (opcode < ILOAD_0) {
				...
				// istore_0, ..., astore_3
				shiftIndex0(ci, index, opcode, lessThan, delta, ISTORE_0,
						ISTORE);
			}
		} else if (opcode == IINC) {
	...
	private static void shiftIndex8(CodeIterator ci, int index, int opcode,
			int lessThan, int delta) throws BadBytecode {
		int var = ci.byteAt(index + 1);
		if (var < lessThan)
			return;
	...
	private static void shiftIndex0(CodeIterator ci, int index, int opcode,
			int lessThan, int delta, int opcode_i_0, int opcode_i)
			throws BadBytecode {
