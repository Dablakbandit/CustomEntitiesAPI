...
public class InstructionPrinter implements Opcode {
	 ...
	 * position.
	 */
	public static String instructionString(CodeIterator iter, int pos,
			ConstPool pool) {
		...
		case INVOKEINTERFACE:
			return opstring + " "
					+ interfaceMethodInfo(pool, iter.u16bitAt(pos + 1));
		case INVOKEDYNAMIC:
	...
	private static String lookupSwitch(CodeIterator iter, int pos) {
		...
		for (; index < end; index += 8) {
			...
			buffer.append("\t\t").append(match).append(": ").append(target)
					.append("\n");
		}
	...
	private static String tableSwitch(CodeIterator iter, int pos) {
		...
		for (int key = low; index < end; index += 4, key++) {
			...
			buffer.append("\t\t").append(key).append(": ").append(target)
					.append("\n");
		}
