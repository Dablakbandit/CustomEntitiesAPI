...
public class StackMapTable extends AttributeInfo {
	...
	public AttributeInfo copy(ConstPool newCp, Map classnames)
		...
		try {
			return new StackMapTable(newCp, new Copier(this.constPool, info,
					newCp, classnames).doit());
		} catch (BadBytecode e) {
	...
	static class SimpleCopy extends Walker {
		...
		public void sameLocals(int pos, int offsetDelta, int stackTag,
				int stackData) {
			writer.sameLocals(offsetDelta, stackTag,
					copyData(stackTag, stackData));
		}
		...
		public void fullFrame(int pos, int offsetDelta, int[] localTags,
				int[] localData, int[] stackTags, int[] stackData) {
			writer.fullFrame(offsetDelta, localTags,
					copyData(localTags, localData), stackTags,
	...
	static class InsertLocal extends SimpleCopy {
		...
		public void fullFrame(int pos, int offsetDelta, int[] localTags,
			...
			if (len < varIndex) {
				super.fullFrame(pos, offsetDelta, localTags, localData,
						stackTags, stackData);
				return;
			}
			...
			super.fullFrame(pos, offsetDelta, localTags2, localData2,
					stackTags, stackData);
		}
	}
	...
	public static class Writer {
		 ...
		 * @param data
		 *            <code>stack[0].cpool_index</code> if the tag is
		 *            <code>OBJECT</code>, or <cod>stack[0].offset</code> if the
		 ...
		 *            <code>OBJECT</code>, or <cod>locals[].offset</code> if the
		 *            tag is <code>UNINIT</code>. Otherwise, this parameter is
		 *            not used.
		 */
		 ...
		 *            <code>OBJECT</code>, or <cod>stack[].offset</code> if the
		 *            tag is <code>UNINIT</code>. Otherwise, this parameter is
		 *            not used.
		 */
		public void fullFrame(int offsetDelta, int[] localTags,
				int[] localData, int[] stackTags, int[] stackData) {
			numOfEntries++;
	...
	static class Printer extends Walker {
		...
		public void chopFrame(int pos, int offsetDelta, int k) {
			offset += offsetDelta + 1;
			writer.println(offset + " chop frame: " + offsetDelta + ",    " + k
					+ " last locals");
		}
	 ...
	 * 
	 * <p>
	 * This method is for ja.convert.TransformNew. It is called to update the
	 * stack map table when the NEW opcode (and the following DUP) is removed.
	 * 
	 * @param where
		...
		public void fullFrame(int pos, int offsetDelta, int[] localTags,
			...
			for (int i = 0; i < n; i++)
				if (stackTags[i] == UNINIT && stackData[i] == posOfNew
						&& stackTags[i + 1] == UNINIT
						&& stackData[i + 1] == posOfNew) {
					n++;
			...
			super.fullFrame(pos, offsetDelta, localTags, localData, stackTags,
					stackData);
		}
	}
}
