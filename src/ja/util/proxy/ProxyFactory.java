 ...
 * 
 * <p>
 * This factory generates a class that extends the given super class and
 * implements the given interfaces. The calls of the methods inherited from the
 * super class are forwarded and then <code>invoke()</code> is called on the
 * method handler associated with instances of the generated class. The calls of
 * the methods from the interfaces are also forwarded to the method handler.
 * 
 * <p>
 ...
 * <code>mi</code> and prints a message before executing the originally called
 * method <code>bar()</code> in <code>Foo</code>.
 * 
 * <ul>
 * 
 * <pre>
 ...
 * instantiates it, and sets the method handler of the instance:
 * 
 * <ul>
 * 
 * <pre>
 ...
 * 
 * <p>
 * If setHandler is never called for a proxy instance then it will employ the
 * default handler which proceeds by invoking the original method. The behaviour
 * of the default handler is identical to the following handler:
 * 
 * <ul>
 * 
 * <pre>
 ...
 * 
 * <p>
 * A proxy factory caches and reuses proxy classes by default. It is possible to
 * reset this default globally by setting static field
 * {@link ProxyFactory#useCache} to false. Caching may also be configured for a
 * specific factory by calling instance method
 * {@link ProxyFactory#setUseCache(boolean)}. It is strongly recommended that
 * new clients of class ProxyFactory enable caching. Failure to do so may lead
 * to exhaustion of the heap memory area used to store classes.
 * 
 * <p>
 * Caching is automatically disabled for any given proxy factory if deprecated
 * instance method {@link ProxyFactory#setHandler(MethodHandler)} is called.
 * This method was used to specify a default handler which newly created proxy
 * classes should install when they create their instances. It is only retained
 * to provide backward compatibility with previous releases of ja.
 * Unfortunately,this legacy behaviour makes caching and reuse of proxy classes
 * impossible. The current programming model expects ja clients to set the
 * handler of a proxy instance explicitly by calling method
 * {@link Proxy#setHandler(MethodHandler)} as shown in the sample code above.
 * New clients are strongly recommended to use this model rather than calling
 * {@link ProxyFactory#setHandler(MethodHandler)}.
 * 
 * <p>
 * A proxy object generated by <code>ProxyFactory</code> is serializable if its
 * super class or any of its interfaces implement
 * <code>java.io.Serializable</code>. However, a serialized proxy object may not
 * be compatible with future releases. The serialization support should be used
 * for short-term storage or RMI.
 * 
 * <p>
 * For compatibility with older releases serialization of proxy objects is
 * implemented by adding a writeReplace method to the proxy class. This allows a
 * proxy to be serialized to a conventional {@link java.io.ObjectOutputStream}
 * and deserialized from a corresponding {@link java.io.ObjectInputStream}.
 * However this method suffers from several problems, the most notable one being
 * that it fails to serialize state inherited from the proxy's superclass.
 * <p>
 * An alternative method of serializing proxy objects is available which fixes
 * these problems. It requires inhibiting generation of the writeReplace method
 * and instead using instances of {@link ja.util.proxy.ProxyObjectOutputStream}
 * and {@link ja.util.proxy.ProxyObjectInputStream} (which are subclasses of
 * {@link java.io.ObjectOutputStream} and {@link java.io.ObjectInputStream}) to
 * serialize and deserialize, respectively, the proxy. These streams recognise
 * ja proxies and ensure that they are serialized and deserialized without the
 * need for the proxy class to implement special methods such as writeReplace.
 * Generation of the writeReplace method can be disabled globally by setting
 * static field {@link ProxyFactory#useWriteReplace} to false. Alternatively, it
 * may be configured per factory by calling instance method
...
public class ProxyFactory {
	 ...
	 * generated proxy class is written under the directory specified by this
	 * variable. For example, if the value is <code>"."</code>, then the class
	 * file is written under the current directory. This method is for
	 * debugging.
	 * 
	 * <p>
	...
	/**
	 * If true, a generated proxy class is cached and it will be reused when
	 * generating the proxy class with the same properties is requested. The
	 * default value is true.
	 * 
	 * Note that this value merely specifies the initial setting employed by any
	 * newly created proxy factory. The factory setting may be overwritten by
	 * calling factory instance method {@link #setUseCache(boolean)}
	 * 
	 * @since 3.4
	 */
	...
	/**
	 * If true, a generated proxy class will implement method writeReplace
	 * enabling serialization of its proxies to a conventional
	 * ObjectOutputStream. this (default) setting retains the old ja behaviour
	 * which has the advantage that it retains compatibility with older releases
	 * and requires no extra work on the part of the client performing the
	 * serialization. However, it has the disadvantage that state inherited from
	 * the superclasses of the proxy is lost during serialization. if false then
	 * serialization/deserialization of the proxy instances will preserve all
	 * fields. However, serialization must be performed via a
	 * {@link ProxyObjectOutputStream} and deserialization must be via
	 * {@link ProxyObjectInputStream}. Any attempt to serialize proxies whose
	 * class was created with useWriteReplace set to false via a normal
	 * {@link java.io.ObjectOutputStream} will fail.
	 * 
	 * Note that this value merely specifies the initial setting employed by any
	 * newly created proxy factory. The factory setting may be overwritten by
	 * calling factory instance method {@link #setUseWriteReplace(boolean)}
	 * 
	 * @since 3.4
	 */
	...
	/*
	 * methods allowing individual factory settings for factoryUseCache and
	 * factoryWriteReplace to be reset
	 */
	 ...
	 * 
	 * @param useCache
	 *            true if this factory should use the proxy cache and false if
	 *            it should not use the cache
	 * @throws RuntimeException
	 *             if a default interceptor has been set for the factory
	 */
	public void setUseCache(boolean useCache) {
		// we cannot allow caching to be used if the factory is configured to
		// install a default interceptor
		// field into generated classes
		if (handler != null && useCache) {
			throw new RuntimeException(
					"caching cannot be enabled if the factory default interceptor has been set");
		}
		factoryUseCache = useCache;
	}

	/**
	 * test whether this factory installs a writeReplace method in created
	 * classes
	 * 
	 * @return true if this factory installs a writeReplace method in created
	...
	/**
	 * configure whether this factory should add a writeReplace method to
	 * created classes
	 * 
	 * @param useWriteReplace
	 *            true if this factory should add a writeReplace method to
	 *            created classes and false if it should not add a writeReplace
	 *            method
	 */
	...
	/**
	 * used to store details of a specific proxy class in the second tier of the
	 * proxy cache. this entry will be located in a hashmap keyed by the unique
	 * identifying name of the proxy class. the hashmap is located in a weak
	 * hashmap keyed by the classloader common to all proxy classes in the
	 * second tier map.
	 */
	static class ProxyDetails {
		/**
		 * the unique signature of any method filter whose behaviour will be met
		 * by this class. each bit in the byte array is set if the filter
		 * redirects the corresponding super or interface method and clear if it
		 * does not redirect it.
		 */
		byte[] signature;
		/**
		 * a hexadecimal string representation of the signature bit sequence.
		 * this string also forms part of the proxy class name.
		 */
		WeakReference proxyClass;
		/**
		 * a flag which is true this class employs writeReplace to perform
		 * serialization of its instances and false if serialization must employ
		 * of a ProxyObjectOutputStream and ProxyObjectInputStream
		 */
		boolean isUseWriteReplace;

		ProxyDetails(byte[] signature, Class proxyClass,
				boolean isUseWriteReplace) {
	...
	/**
	 * Sets a filter that selects the methods that will be controlled by a
	 * handler.
	 */
	...
	/**
	 * Generates a proxy class with a specific signature. access is package
	 * local so ProxyObjectInputStream can use this
	 * 
	 * @param signature
	 * @return
	 */
	...
	public String getKey(Class superClass, Class[] interfaces,
			byte[] signature, boolean useWriteReplace) {
	...
	private void createClass2(ClassLoader cl) {
		String key = getKey(superClass, interfaces, signature,
				factoryWriteReplace);
		/*
	...
	private void createClass3(ClassLoader cl) {
		...
		try {
			...
			// legacy behaviour : we only set the default interceptor static
			// field if we are not using the cache
			if (!factoryUseCache) {
	 ...
	 * object is called to obtain a class loader.
	 * 
	 * <p>
	 ...
	 * specified.
	 * 
	 * @deprecated since 3.12 use of this method is incompatible with proxy
	 *             class caching. instead clients should call method
	 *             {@link Proxy#setHandler(MethodHandler)} to set the handler
	 *             for each newly created proxy instance. calling this method
	 *             will automatically disable caching of classes created by the
	 *             proxy factory.
	 */
	public void setHandler(MethodHandler mi) {
		// if we were using the cache and the handler is non-null then we must
		// stop caching
		if (factoryUseCache && mi != null) {
			factoryUseCache = false;
			// clear any currently held class so we don't try to reuse it or set
			// its handler field
			thisClass = null;
		}
		handler = mi;
		// this retains the behaviour of the old code which resets any class we
		// were holding on to
	 ...
	 * algorithm to generate a unique name. The <code>get</code> method does not
	...
	public static UniqueName nameGenerator = new UniqueName() {
		private final String sep = "_$$_jvst"
				+ Integer.toHexString(this.hashCode() & 0xfff) + "_";
	...
	private ClassFile make() throws CannotCompileException {
		...
		setInterfaces(cf, interfaces, hasGetHandler ? Proxy.class
				: ProxyObject.class);
		...
		// caching is disabled
		if (!factoryUseCache) {
			FieldInfo finfo = new FieldInfo(pool, DEFAULT_INTERCEPTOR,
					HANDLER_TYPE);
		...
		FieldInfo finfo3 = new FieldInfo(pool, FILTER_SIGNATURE_FIELD,
		...
		FieldInfo finfo4 = new FieldInfo(pool, SERIAL_VERSION_UID_FIELD,
													...
													// CannotCompileException
	{
		...
		if (signature.length != maxBytes) {
			throw new RuntimeException(
					"invalid filter signature length for deserialized proxy class");
		}
	...
	private static void addClassInitializer(ClassFile cf, ConstPool cp,
		...
		code.addInvokestatic("java.lang.Class", "forName",
		...
		code.addPutstatic(classname, SERIAL_VERSION_UID_FIELD,
				SERIAL_VERSION_UID_TYPE);
	 ...
	 *            might be null.
	 */
	private static void callFind2Methods(Bytecode code, String superMethod,
			String thisMethod, int index, String desc, int classVar,
			int arrayVar) {
	...
	private int overrideMethods(ClassFile cf, ConstPool cp, String className,
			ArrayList forwarders) throws CannotCompileException {
	...
	private void override(String thisClassname, Method meth, String prefix,
		...
		if (Modifier.isAbstract(meth.getModifiers()))
			...
			MethodInfo delegator = makeDelegator(meth, desc, cp, declClass,
					delegatorName);
			// delegator is not a bridge method. See Sec. 15.12.4.5 of JLS 3rd
			// Ed.
			delegator.setAccessFlags(delegator.getAccessFlags()
					& ~AccessFlag.BRIDGE);
			cf.addMethod(delegator);
		}

		MethodInfo forwarder = makeForwarder(thisClassname, meth, desc, cp,
				declClass, delegatorName, index, forwarders);
	...
	private void makeConstructors(String thisClassName, ClassFile cf,
		...
		Constructor[] cons = SecurityActions
				.getDeclaredConstructors(superClass);
		...
		for (int i = 0; i < cons.length; i++) {
			...
			if (!Modifier.isFinal(mod) && !Modifier.isPrivate(mod)
				...
				MethodInfo m = makeConstructor(thisClassName, c, cp,
						superClass, doHandlerInit);
				cf.addMethod(m);
			}
		}
	}
	...
	private static MethodInfo makeConstructor(String thisClassName,
		...
		// legacy: if we are not using caching then we initialise the instance's
		// handler
		// from the class's static default interceptor and skip the next few
		// instructions if
		// it is non-null
		if (doHandlerInit) {
			code.addAload(0);
		...
		code.addAload(0);
		code.addGetstatic(NULL_INTERCEPTOR_HOLDER, DEFAULT_INTERCEPTOR,
				HANDLER_TYPE);
		code.addPutfield(thisClassName, HANDLER, HANDLER_TYPE);
	 ...
	 */
	private static MethodInfo makeForwarder(String thisClassName, Method meth,
			String desc, ConstPool cp, Class declClass, String delegatorName,
			int index, ArrayList forwarders) {
		MethodInfo forwarder = new MethodInfo(cp, meth.getName(), desc);
		forwarder
				.setAccessFlags(Modifier.FINAL
		...
		/*
		 * static { methods[index * 2] = RuntimeSupport.findSuperMethod(this,
		 * <overridden name>, <desc>); methods[index * 2 + 1] =
		 * RuntimeSupport.findMethod(this, <delegator name>, <desc>); or = null
		 * // the original method is abstract. } : return
		 * ($r)handler.invoke(this, methods[index * 2], methods[index * 2 + 1],
		 * $args);
		 */
		...
		forwarders.add(new Find2MethodsArgs(meth.getName(), delegatorName,
				desc, origIndex));
