...
public class Reflection implements Translator {
	...
	static final String classMetaobjectClassName = "ja.tools.reflect.ClassMetaobject";
	...
	private boolean isExcluded(String name) {
		return name.startsWith(ClassMetaobject.methodPrefix)
				...
				|| name.equals(metaobjectGetter) || name.startsWith(readPrefix)
	...
	public void start(ClassPool pool) throws NotFoundException {
		classPool = pool;
		final String msg = "ja.tools.reflect.Sample is not found or broken.";
		try {
	...
	/**
	 * Inserts hooks for intercepting accesses to the fields declared in
	 * reflective classes.
	 */
	...
	/**
	 * Produces a reflective class. If the super class is also made reflective,
	 * it must be done before the sub class.
	 * 
	 * @param classname
	 ...
	 * @see ja.tools.reflect.ClassMetaobject
	 */
	public boolean makeReflective(String classname, String metaobject,
			String metaclass) throws CannotCompileException, NotFoundException {
	 ...
	 * @param metaobject
	 *            the class of metaobjects. It must be a subclass of
	...
	/**
	 * Produces a reflective class. It modifies the given <code>CtClass</code>
	 * object and makes it reflective. If the super class is also made
	 * reflective, it must be done before the sub class.
	 * 
	 * @param clazz
	 *            the reflective class.
	 * @param metaobject
	 ...
	 * @see ja.tools.reflect.ClassMetaobject
	 */
	public boolean makeReflective(CtClass clazz, CtClass metaobject,
			CtClass metaclass) throws CannotCompileException,
			CannotReflectException, NotFoundException {
		if (clazz.isInterface())
			throw new CannotReflectException("Cannot reflect an interface: "
					+ clazz.getName());
	...
	/**
	 * Registers a reflective class. The field accesses to the instances of this
	 * class are instrumented.
	 */
	...
	private boolean modifyClassfile(CtClass clazz, CtClass metaobject,
		...
		clazz.addField(
				f,
				CtField.Initializer.byNew(metaclass,
	...
	private void processMethods0(int mod, CtClass clazz, CtMethod m,
			int identifier, boolean dontSearch) throws CannotCompileException,
			NotFoundException {
		CtMethod body;
		...
		CtMethod wmethod = CtNewMethod.wrapped(m.getReturnType(), name,
				m.getParameterTypes(), m.getExceptionTypes(), body,
				ConstParameter.integer(identifier), clazz);
	...
	private void processFields(CtClass clazz) throws CannotCompileException,
			NotFoundException {
		CtField[] fs = clazz.getDeclaredFields();
		for (int i = 0; i < fs.length; ++i) {
			CtField f = fs[i];
			...
			if ((mod & Modifier.PUBLIC) != 0 && (mod & Modifier.FINAL) == 0) {
				...
				CtMethod wmethod = CtNewMethod.wrapped(ftype,
						readPrefix + name, readParam, null, trapRead,
				...
				writeParam[1] = ftype;
				wmethod = CtNewMethod.wrapped(CtClass.voidType, writePrefix
						+ name, writeParam, null, trapWrite,
						ConstParameter.string(name), clazz);
				wmethod.setModifiers(mod);
				clazz.addMethod(wmethod);
			}
