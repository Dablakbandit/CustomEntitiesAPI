 ...
 * 		ja.tools.reflect.Loader cl = (ja.tools.reflect.Loader) Main.class
 * 				.getClassLoader();
 ...
 * 
 * <pre>
 * % java ja.tools.reflect.Loader Main arg1, ...
 * </pre>
 * 
 * </ul>
 * 
 ...
 * loaded by <code>ja.tools.reflect.Loader</code> whereas the class
 * <code>Main2</code> is not. Thus, <code>Main</code> belongs to the same name
 * space (security domain) as <code>MyApp</code> whereas <code>Main2</code> does
 * not; <code>Main2</code> belongs to the same name space as
 * <code>ja.tools.reflect.Loader</code>. For more details, see the notes in the
 * manual page of <code>ja.Loader</code>.
 * 
 * <p>
 ...
 * 		ja.Loader cl = new ja.Loader(ClassPool.getDefault(reflection));
 ...
 * 
 * <p>
 * <code>ja.tools.reflect.Loader</code> does not make a class reflective if that
 * class is in a <code>java.*</code> or <code>javax.*</code> pacakge because of
 * the specifications on the class loading algorithm of Java. The JVM does not
 * allow to load such a system class with a user class loader.
 * 
 * <p>
 * To avoid this limitation, those classes should be statically modified with
 * <code>ja.tools.reflect.Compiler</code> and the original class files should be
 * replaced.
 * 
...
public class Loader extends ja.Loader {
	 ...
	 */
	public boolean makeReflective(String clazz, String metaobject,
