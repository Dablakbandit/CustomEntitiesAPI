 ...
 * to that object. The class name of the proxy object is identical to that of
 * the remote object. The proxy object provides the same set of methods as the
 * remote object. If one of the methods is invoked on the proxy object, the
 * invocation is delegated to the remote object. From the viewpoint of the
 ...
 * <code>RuntimeException</code>, the caller method does not need to catch the
 * exception. However, good programs should catch the
...
public class ObjectImporter implements java.io.Serializable {
	 ...
	 * 
	 * <p>
	 * If you run a program with <code>ja.tools.web.Viewer</code>, you can
	 * construct an object importer as follows:
	 * 
	 * <ul>
	 * 
	 * <pre>
	...
	/**
	 * Finds the object exported by a server with the specified name. If the
	 * object is not found, this method returns null.
	 * 
	 * @param name
	...
	/**
	 * Finds the object exported by the server with the specified name. It sends
	 * a POST request to the server (via an http proxy server if needed).
	 * 
	 * @param name
	 *            the name of the exported object.
	...
	public Object call(int objectid, int methodid, Object[] args)
		...
		try {
			/*
			 * This method establishes a raw tcp connection for sending a POST
			 * message. Thus the object cannot communicate a remote object
