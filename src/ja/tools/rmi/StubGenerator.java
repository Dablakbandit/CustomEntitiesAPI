...
public class StubGenerator implements Translator {
	...
	/**
	 * Initializes the object. This is a method declared in ja.Translator.
	 * 
	...
	public void start(ClassPool pool) throws NotFoundException {
		...
		proxyConstructorParamTypes = pool.get(new String[] {
				"ja.tools.rmi.ObjectImporter", "int" });
		interfacesForProxy = pool.get(new String[] { "java.io.Serializable",
				"ja.tools.rmi.Proxy" });
		exceptionForProxy = new CtClass[] { pool
				.get("ja.tools.rmi.RemoteException") };
	}

	/**
	 * Does nothing. This is a method declared in ja.Translator.
	 * 
	...
	private CtClass produceProxyClass(CtClass orgclass, Class orgRtClass)
		...
		CtField f = new CtField(classPool.get("ja.tools.rmi.ObjectImporter"),
				fieldImporter, proxy);
		...
		CtConstructor cons = CtNewConstructor.skeleton(
				proxyConstructorParamTypes, null, proxy);
		...
		for (int i = 0; i < ms.length; ++i) {
			...
			if (m.getDeclaringClass() != Object.class && !Modifier.isFinal(mod))
				if (Modifier.isPublic(mod)) {
					...
					wmethod = CtNewMethod.wrapped(toCtClass(m.getReturnType()),
							m.getName(), toCtClass(m.getParameterTypes()),
							exceptionForProxy, body, ConstParameter.integer(i),
							proxy);
	...
	private void modifySuperclass(CtClass orgclass)
		...
		for (;; orgclass = superclazz) {
			...
			superclazz.addConstructor(CtNewConstructor
					.defaultConstructor(superclazz));
		}
	}
}
