...
/**
 * A container of <code>CtClass</code> objects. A <code>CtClass</code> object
 * must be obtained from this object. If <code>get()</code> is called on this
 ...
 * guaranteed. Thus if a large number of <code>CtClass</code>es are processed,
 * the <code>ClassPool</code> will consume a huge amount of memory. To avoid
 * this, a <code>ClassPool</code> object should be recreated, for example, every
 ...
 * pool, <code>get()</code> first asks the parent pool to find a class file.
 * Only if the parent could not find the class file, <code>get()</code> searches
 * the <code>ClassPath</code>s of the child <code>ClassPool</code>. This search
 * order is reversed if <code>ClassPath.childFirstLookup</code> is
 * <code>true</code>.
 * 
 * @see ja.CtClass
 * @see ja.ClassPath
...
public class ClassPool {
	 ...
	 * <code>appendSystemPath()</code> is called. Otherwise, this constructor is
	 * equivalent to the constructor taking no parameter.
	 * 
	 ...
	 * 
	 * @param parent
	 *            the parent of this class pool. If this is a root class pool,
	 ...
	 * 
	 * <ul>
	 * <code>ClassPool cp = new ClassPool();
	 * cp.appendSystemPath();
	 * </code>
	 * </ul>
	 * 
	...
	/**
	 * This method is periodically invoked so that memory footprint will be
	 * minimized.
	 */
	void compress() {
	...
	/**
	 * Record a package name so that the ja compiler searches the package to
	 * resolve a class name. Don't record the <code>java.lang</code> package,
	 ...
	 * 
	 * @param packageName
	 *            the package name. It must not include the last '.' (dot). For
	 *            example, "java.util" is valid but "java.util." is wrong.
	 * @since 3.1
	 */
	...
	/**
	 * Records a class name that never exists. For example, a package name can
	 * be recorded by this method. This would improve execution performance
	...
	/**
	 * Records the <code>$cflow</code> variable for the field specified by
	 * <code>cname</code> and <code>fname</code>.
	 * 
	 * @param name
	 *            variable name
	 * @param cname
	...
	/**
	 * Reads a class file and constructs a <code>CtClass</code> object with a
	 * new name. This method is useful if you want to generate a new class as a
	 * copy of another class (except the class name). For example,
	 * 
	 * <ul>
	 * 
	 * <pre>
	 ...
	 * <code>Point</code> class except the class name since <code>Pair</code> is
	 * defined by reading <code>Point.class</code>.
	 * 
	 * @param orgName
	...
	/*
	 * This method is invoked by CtClassType.setName(). It removes a CtClass
	 * object from the hash table and inserts it with the new name. Don't
	 ...
	 * <code>CtClass</code> object representing that class file. If that class
	...
	/**
	 * Reads a class file from the source and returns a reference to the
	 * <code>CtClass</code> object representing that class file. This method is
	 * equivalent to <code>get</code> except that it returns <code>null</code>
	 * when a class file is not found and it never throws an exception.
	 * 
	 * @param classname
	 *            a fully-qualified class name.
	...
	public CtClass getOrNull(String classname) {
		...
		if (classname == null)
			...
			try {
				/*
				 * ClassPool.get0() never throws an exception but its subclass
				 * may implement get0 that may throw an exception.
				 */
				clazz = get0(classname, true);
	...
	/**
	 * Returns a <code>CtClass</code> object with the given name. This is almost
	 ...
	 * <code>getClassInfo</code> in <code>ja.bytecode.ClassPool</code>.
	 * <code>getClassInfo</code> returns a fully-qualified class name but, if
	 * the class is an array type, it returns a descriptor.
	 * 
	 * @param classname
	 *            a fully-qualified class name or a descriptor representing an
	 *            array type.
	 * @see #get(String)
	...
	/**
	 * Creates a CtClass object representing the specified class. It first
	 * examines whether or not the corresponding class file exists. If yes, it
	 * creates a CtClass object.
	 * 
	 ...
	 * classname. It is also used to determine whether the class file exists.
	 * 
	 * @param classname
	 *            a fully-qualified class name.
	 * @return null if the class file could not be found.
	 ...
	 * file; you do not have to explicitly give the name.
	 * 
	 * @param classfile
	 *            class file.
	 ...
	 */
	public CtClass makeClass(InputStream classfile) throws IOException,
			RuntimeException {
	 ...
	 *            class file.
	 * @param ifNotFrozen
	 *            throws a RuntimeException if this parameter is true and there
	 *            is a frozen class with the same name.
	 * @see ja.ByteArrayClassPath
	 */
	 ...
	 * @since 3.9
	 */
	public CtClass makeClassIfNew(InputStream classfile) throws IOException,
			RuntimeException {
		compress();
		classfile = new BufferedInputStream(classfile);
		CtClass clazz = new CtClassType(classfile, this);
	 ...
	 * 
	 * <p>
	 * If no constructor is explicitly added to the created new class, ja
	 * generates constructors and adds it when the class file is generated. It
	 * generates a new constructor for each constructor of the super class. The
	 * new constructor takes the same set of parameters and invokes the
	 * corresponding constructor of the super class. All the received parameters
	 * are passed to it.
	 * 
	 * @param classname
	 *            a fully-qualified class name.
	...
	/**
	 * Creates a new public interface. If there already exists a class/interface
	 * with the same name, the new interface overwrites that previous one.
	 * 
	 * @param name
	...
	/**
	 * Inserts a directory or a jar (or zip) file at the head of the search
	 * path.
	 * 
	 * @param pathname
	...
	/**
	 * Detatches the <code>ClassPath</code> object from the search path. The
	 * detached <code>ClassPath</code> object cannot be added to the path again.
	 */
	 ...
	 * 
	 * @param pathlist
	 *            a (semi)colon-separated list of the path names of directories
	 *            and jar files. The directory name must not end with a path
	 *            separator ("/").
	 * @throws NotFoundException
	...
	/**
	 * Converts the given class to a <code>java.lang.Class</code> object. Once
	...
	public Class toClass(CtClass clazz) throws CannotCompileException {
		// Some subclasses of ClassPool may override
		// toClass(CtClass,ClassLoader).
		// So we should call that method instead of toClass(..,
		// ProtectionDomain).
	 ...
	 *             {@link #toClass(CtClass,ClassLoader,ProtectionDomain)}. A
	 *             subclass of <code>ClassPool</code> that has been overriding
	 *             this method should be modified. It should override
	 ...
	 * 
	 * @param loader
	 *            the class loader used to load this class. For example, the
	 *            loader returned by <code>getClassLoader()</code> can be used
	 *            for this parameter.
	 * @param domain
	...
	private static synchronized Object toClass2(Method method,
			ClassLoader loader, Object[] args) throws Exception {
	 ...
	 * returned by <code>toClass()</code> will return a non-null object.
	 * 
	 * @param loader
	 *            the class loader passed to <code>toClass()</code> or the
	...
	public void makePackage(ClassLoader loader, String name)
			throws CannotCompileException {
		Object[] args = new Object[] { name, null, null, null, null, null,
				null, null };
		Throwable t;
		try {
			...
			if (t == null)
				...
				// if the package is already defined, an
