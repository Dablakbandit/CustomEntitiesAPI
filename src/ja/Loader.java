 ...
 * 
 * <p>
 * Note that ja can be used without this class loader; programmers can define
 * their own versions of class loader. They can run a program even without any
 * user-defined class loader if that program is statically translated with ja.
 ...
 * The startup program of an application using <code>MyTranslator</code> should
 ...
 * example, if only a class <code>test.Rectangle</code> is modified, the
 * <code>main()</code> method above will be the following:
 * 
 * <ul>
 * 
 * <pre>
 ...
 * <code>org.xml.*</code>, ...) unless <code>Loader.doDelegation</code> is
 * <code>false</code>. This is because the JVM prohibits a user class loader
 ...
 * <code>CL</code>, all classes that the class <code>C</code> refers to are also
 * loaded by <code>CL</code>. However, if <code>CL</code> delegates the loading
 ...
 * <code>ja.Loader</code> and classes included in package <code>java.*</code>
 * and <code>javax.*</code> to the parent class loader. Other classes are
 * directly loaded by this loader.
 * 
 * <p>
 ...
 * loader. If the constructor of <code>java.io.File</code> is called with an
 * instance of <code>java.lang.String</code>, then it may throw an exception
 * since it accepts an instance of only the <code>java.lang.String</code> loaded
...
public class Loader extends ClassLoader {
	 ...
	 * Without registering an appropriate protection domain, the program loaded
	 * by this loader will not work with a security manager or a signed jar
	 * file.
	 */
	 ...
	 * 
	 * @param cp
	 *            the <code>ClassPool</code> object for obtaining a class file.
	 * @param t
	 *            a translator.
	 ...
	 *            <code>args[1..n]</code> are parameters passed to the target
	 *            <code>main()</code>.
	 *            </ul>
	 * 
	 ...
	 *             if an exception is thrown while obtaining a class file.
	 */
	protected Class findClass(String name) throws ClassNotFoundException {
		...
		if (i != -1) {
			...
			if (getPackage(pname) == null)
				try {
					definePackage(pname, null, null, null, null, null, null,
							null);
	...
	protected Class loadClassByDelegation(String name)
			throws ClassNotFoundException {
		/*
		 * The swing components must be loaded by a system class loader.
		 * javax.swing.UIManager loads a (concrete) subclass of LookAndFeel by a
		 * system class loader and cast an instance of the class to LookAndFeel
		 * for (maybe) a security reason. To avoid failure of type conversion,
		 * LookAndFeel must not be loaded by this class loader.
		 */
		...
		if (doDelegation)
			if (name.startsWith("java.") || name.startsWith("javax.")
					...
					|| name.startsWith("org.xml.") || notDelegated(name))
				c = delegateToParent(name);
