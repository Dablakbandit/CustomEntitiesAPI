...
public class JvstCodeGen extends MemberCodeGen {
	...
	protected void atCflow(ASTList cname) throws CompileError {
		...
		bytecode.addGetstatic((String) names[0], (String) names[1],
				"Lja/runtime/Cflow;");
		bytecode.addInvokevirtual("ja.runtime.Cflow", "value", "()I");
		exprType = INT;
		arrayDim = 0;
		className = null;
	}
	...
	public boolean isParamListName(ASTList args) {
		if (paramTypeList != null && args != null && args.tail() == null) {
			...
			return (left instanceof Member && ((Member) left).get().equals(
					paramListName));
		} else
			return false;
	}
	...
	void compileInvokeSpecial(ASTree target, String classname,
			String methodname, String descriptor, ASTList args)
			throws CompileError {
		target.accept(this);
	 ...
	 */
	public int recordParams(CtClass[] params, boolean isStatic, String prefix,
			String paramVarName, String paramsName, SymbolTable tbl)
			throws CompileError {
		return recordParams(params, isStatic, prefix, paramVarName, paramsName,
				!isStatic, 0, getThisName(), tbl);
	}
	 ...
	 */
	public int recordParams(CtClass[] params, boolean isStatic, String prefix,
			String paramVarName, String paramsName, boolean use0,
		...
		if (use0) {
			...
			Declarator decl = new Declarator(CLASS,
					MemberResolver.javaToJvmName(target), 0, varNo++,
					new Symbol(varName));
	...
	private int recordVar(CtClass cc, String varName, int varNo, SymbolTable tbl)
		...
		Declarator decl = new Declarator(exprType, className, arrayDim, varNo,
				new Symbol(varName));
		tbl.append(varName, decl);
	...
	public void recordVariable(String typeDesc, String varName, int varNo,
		...
		Declarator decl = new Declarator(type, cname, dim, varNo, new Symbol(
				varName));
		tbl.append(varName, decl);
	}
	 ...
	 */
	public static int compileParameterList(Bytecode code, CtClass[] params,
			int regno) {
		if (params == null) {
