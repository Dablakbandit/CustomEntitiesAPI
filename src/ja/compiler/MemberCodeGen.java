...
public class MemberCodeGen extends CodeGen {
	...
	public void atNewExpr(NewExpr expr) throws CompileError {
		if (expr.isArray())
			...
			atMethodCallCore(clazz, MethodInfo.nameInit, args, false, true, -1,
					null);
	...
	public void atNewArrayExpr(NewExpr expr) throws CompileError {
		...
		atNewArrayExpr2(type, sizeExpr,
				Declarator.astToClassName(classname, '/'), init);
	}
	...
	public void atMethodCallCore(CtClass targetClass, String mname,
			ASTList args, boolean isStatic, boolean isSpecial, int aload0pos,
		...
		atMethodCallCore2(targetClass, mname, isStatic, isSpecial, aload0pos,
				count, found);
	}

	private void atMethodCallCore2(CtClass targetClass, String mname,
			boolean isStatic, boolean isSpecial, int aload0pos, int count,
			MemberResolver.Method found) throws CompileError {
		...
		if (mname.equals(MethodInfo.nameInit)) {
			...
			if (declClass == thisClass)
				...
				mname = getAccessiblePrivate(mname, origDesc, desc, minfo,
						declClass);
			}
	...
	protected String getAccessiblePrivate(String methodName, String desc,
			String newDesc, MethodInfo minfo, CtClass declClass)
			throws CompileError {
		if (isEnclosing(declClass, thisClass)) {
			AccessorMaker maker = declClass.getAccessorMaker();
			if (maker != null)
				return maker
						.getMethodAccessor(methodName, desc, newDesc, minfo);
		}
	...
	protected String getAccessibleConstructor(String desc, CtClass declClass,
			MethodInfo minfo) throws CompileError {
		if (isEnclosing(declClass, thisClass)) {
			AccessorMaker maker = declClass.getAccessorMaker();
			if (maker != null)
	...
	protected CtField fieldAccess(ASTree expr, boolean acceptLength)
		...
		if (expr instanceof Member) {
			...
			if (!is_static)
				if (inStaticMethod)
					throw new CompileError("not available in a static method: "
							+ name);
				else
					bytecode.addAload(0); // this
			...
			if (op == MEMBER) {
				/*
				 * static member by # (extension by ja) For example, if
