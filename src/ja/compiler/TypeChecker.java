...
public class TypeChecker extends Visitor implements Opcode, TokenId {
	 ...
	 * object.
	 */
	protected static StringBuffer typeToString(StringBuffer sbuf, int type,
			int dim, String cname) {
		String s;
		if (type == CLASS)
	...
	protected String getSuperName() throws CompileError {
		return MemberResolver.javaToJvmName(MemberResolver.getSuperclass(
				thisClass).getName());
	}
	...
	public void atAssignExpr(AssignExpr expr) throws CompileError {
		...
		if (left instanceof Variable)
			atVariableAssign(expr, op, (Variable) left,
					((Variable) left).getDeclarator(), right);
		else {
	...
	private void atVariableAssign(Expr expr, int op, Variable var,
			Declarator d, ASTree right) throws CompileError {
		...
		if (k >= 0) {
			...
			if (token == '+') {
				...
				if (e != null) {
					 ...
					 */
					e = CallExpr.makeCall(
							Expr.make('.', e, new Member("toString")), null);
					expr.setOprand1(e);
	...
	private Expr atPlusExpr(BinExpr expr) throws CompileError {
		...
		if ((type1 == CLASS && dim1 == 0 && jvmJavaLangString.equals(cname))
				|| (exprType == CLASS && arrayDim == 0 && jvmJavaLangString
						.equals(className))) {
			ASTList sbufClass = ASTList.make(new Symbol("java"), new Symbol(
					"lang"), new Symbol("StringBuffer"));
	...
	public static ASTree getConstantFieldValue(CtField f) {
		...
		if (value instanceof String)
			...
			int token = (value instanceof Double) ? DoubleConstant
					: FloatConstant;
			...
			return new Keyword(((Boolean) value).booleanValue() ? TokenId.TRUE
					: TokenId.FALSE);
		else
			return null;
	}
	...
	public void atCallExpr(CallExpr expr) throws CompileError {
		...
		if (method instanceof Member) {
			...
			if (op == MEMBER) // static method
				targetClass = resolver.lookupClass(
						((Symbol) e.oprand1()).get(), false);
			else if (op == '.') {
				...
				try {
					...
					e.setOprand1(new Symbol(MemberResolver
							.jvmToJavaName(className)));
				}
	...
	public MemberResolver.Method atMethodCallCore(CtClass targetClass,
			String mname, ASTList args) throws CompileError {
		...
		MemberResolver.Method found = resolver.lookupMethod(targetClass,
				thisClass, thisMethod, mname, types, dims, cnames);
		if (found == null) {
	...
	protected CtField fieldAccess(ASTree expr) throws CompileError {
		if (expr instanceof Member) {
			...
			if (op == MEMBER) {
				 ...
				 * 
				 * because ja is a field name. However, this is often
	...
	private CtField fieldAccess2(Expr e, String jvmClassName)
			throws CompileError {
