...
public abstract class CodeGen extends Visitor implements Opcode, TokenId {
	...
	public void atStmnt(Stmnt st) throws CompileError {
		...
		if (op == EXPR) {
			...
			hasReturned = false;
			throw new CompileError("sorry, not supported statement: TokenId "
					+ op);
		}
	}
	...
	private void atSwitchStmnt(Stmnt st) throws CompileError {
		...
		for (ASTList list = body; list != null; list = list.tail()) {
			...
			if (op == DEFAULT)
				...
				pairs[ipairs++] = ((long) computeLabel(label.head()) << 32)
	...
	protected void atAssignExpr(AssignExpr expr, boolean doDup)
		...
		if (left instanceof Variable)
			atVariableAssign(expr, op, (Variable) left,
					((Variable) left).getDeclarator(), right, doDup);
		else {
	...
	private void atVariableAssign(Expr expr, int op, Variable var,
			Declarator d, ASTree right, boolean doDup) throws CompileError {
		...
		if (expr == null && right instanceof ArrayInit)
			atArrayVariableAssign((ArrayInit) right, varType, varArray,
					varClass);
		else
	...
	protected abstract void atArrayVariableAssign(ArrayInit init, int varType,
	...
	protected void atAssignCore(Expr expr, int op, ASTree right, int type,
			int dim, String cname) throws CompileError {
	...
	private boolean invalidDim(int srcType, int srcDim, String srcClass,
			int destType, int destDim, String destClass, boolean isCast) {
		if (srcDim != destDim)
			if (srcType == NULL)
				return false;
	...
	static final int[] binOp = { '+', DADD, FADD, LADD, IADD, '-', DSUB, FSUB,
			LSUB, ISUB, '*', DMUL, FMUL, LMUL, IMUL, '/', DDIV, FDIV, LDIV,
			IDIV, '%', DREM, FREM, LREM, IREM, '|', NOP, NOP, LOR, IOR, '^',
			NOP, NOP, LXOR, IXOR, '&', NOP, NOP, LAND, IAND, LSHIFT, NOP, NOP,
			LSHL, ISHL, RSHIFT, NOP, NOP, LSHR, ISHR, ARSHIFT, NOP, NOP, LUSHR,
			IUSHR };
	...
	private void atArithBinExpr(Expr expr, int token, int index, int type1)
		...
		if (token == LSHIFT || token == RSHIFT || token == ARSHIFT)
			if (type2 == INT || type2 == SHORT || type2 == CHAR
					|| type2 == BYTE)
				exprType = type1;
			else
				badTypes(expr);
	...
	private void atStringConcatExpr(Expr expr, int type1, int dim1,
		...
		boolean type2IsString = (type2 == CLASS && jvmJavaLangString
				.equals(className));
	...
	/*
	 * Produces the opcode to branch if the condition is true. The oprand is not
	 * produced.
	 * 
	...
	static int getCompOperator(ASTree expr) throws CompileError {
		if (expr instanceof Expr) {
			...
			if (token == '!')
				return '!';
			else if ((bexpr instanceof BinExpr) && token != OROR
					&& token != ANDAND && token != '&' && token != '|')
				return EQ; // ==, !=, ...
			else
				return token;
		}
	...
	private static final int ifOp[] = { EQ, IF_ICMPEQ, IF_ICMPNE, NEQ,
			IF_ICMPNE, IF_ICMPEQ, LE, IF_ICMPLE, IF_ICMPGT, GE, IF_ICMPGE,
			IF_ICMPLT, '<', IF_ICMPLT, IF_ICMPGE, '>', IF_ICMPGT, IF_ICMPLE };

	private static final int ifOp2[] = { EQ, IFEQ, IFNE, NEQ, IFNE, IFEQ, LE,
			IFLE, IFGT, GE, IFGE, IFLT, '<', IFLT, IFGE, '>', IFGT, IFLE };
	 ...
	 */
	private void compareExpr(boolean branchIf, int token, int type1,
			BinExpr expr) throws CompileError {
		if (arrayDim == 0)
	...
	public void atExpr(Expr expr) throws CompileError {
		...
		if (token == '.') {
			...
			/*
			 * MEMBER ('#') is an extension by ja. The compiler internally uses
			 * # for compiling .class expressions such as "int.class".
			 */
			atFieldRead(expr);
		} else if (token == ARRAY)
	...
	protected void atPlusPlusCore(int dup_code, boolean doDup, int token,
			boolean isPost, Expr expr) throws CompileError {
		int t = exprType;
