...
class CtNewWrappedMethod {
	...
	static Bytecode makeBody(CtClass clazz, ClassFile classfile,
			CtMethod wrappedBody, CtClass[] parameters, CtClass returnType,
	...
	/*
	 * The generated method body does not need a stack map table because it does
	 * not contain a branch instruction.
	 */
	protected static int makeBody0(CtClass clazz, ClassFile classfile,
			CtMethod wrappedBody, boolean isStatic, CtClass[] parameters,
			CtClass returnType, ConstParameter cparam, Bytecode code)
			throws CannotCompileException {
		...
		int stacksize = compileParameterList(code, parameters, (isStatic ? 0
				: 1));
		int stacksize2;
		String desc;
		if (cparam == null) {
		...
		try {
			bodyname = addBodyMethod((CtClassType) clazz, classfile,
					wrappedBody);
			/*
			 * if an exception is thrown below, the method added above should be
			 * removed. (future work :<)
			 */
		} catch (BadBytecode e) {
			throw new CannotCompileException(e);
	...
	private static String addBodyMethod(CtClassType clazz, ClassFile classfile,
			CtMethod src) throws BadBytecode, CannotCompileException {
		...
		if (bodyname == null) {
			...
			MethodInfo body = new MethodInfo(classfile.getConstPool(),
					bodyname, src.getMethodInfo2(), map);
