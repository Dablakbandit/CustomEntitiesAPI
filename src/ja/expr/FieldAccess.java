...
public class FieldAccess extends Expr {
	...
	public CtField getField() throws NotFoundException {
		...
		return cc
				.getField(cp.getFieldrefName(index), cp.getFieldrefType(index));
	}
	...
	public void replace(String statement) throws CannotCompileException {
		...
		try {
			CtClass[] params;
			CtClass retType;
			CtClass fieldType = Descriptor.toCtClass(
					constPool.getFieldrefType(index), thisClass.getClassPool());
			...
			if (read)
				jc.recordProceed(new ProceedForRead(retType, opcode, index,
						paramVar));
			else {
				// because $type is not the return type...
				jc.recordType(fieldType);
				jc.recordProceed(new ProceedForWrite(params[0], opcode, index,
						paramVar));
			}
