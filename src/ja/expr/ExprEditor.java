...
public class ExprEditor {
	...
	boolean doit(CtClass clazz, MethodInfo minfo, LoopContext context,
			CodeIterator iterator, int endPos) throws CannotCompileException {
		boolean edited = false;
		...
		try {
			...
			if (c < Opcode.GETSTATIC) // c < 178
				...
				if (c == Opcode.INVOKESTATIC || c == Opcode.INVOKEINTERFACE
					...
					context.newList = new NewOp(context.newList, pos, minfo
							.getConstPool().getClassInfo(index));
					...
					if (newList != null
						...
						MethodCall mcall = new MethodCall(pos, iterator, clazz,
								minfo);
						if (mcall.getMethodName().equals(MethodInfo.nameInit)) {
							ConstructorCall ccall = new ConstructorCall(pos,
									iterator, clazz, minfo);
							expr = ccall;
							edit(ccall);
						} else {
							expr = mcall;
