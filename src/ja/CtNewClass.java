...
public class CtNewClass extends CtClassType {
	...
	CtNewClass(String name, ClassPool cp, boolean isInterface,
			CtClass superclass) {
		super(name, cp);
		wasChanged = true;
		String superName;
	...
	public void inheritAllConstructors() throws CannotCompileException,
		...
		for (int i = 0; i < cs.length; ++i) {
			...
			if (isInheritable(mod, superclazz)) {
				CtConstructor cons = CtNewConstructor.make(
						c.getParameterTypes(), c.getExceptionTypes(), this);
		...
		if (n < 1)
			throw new CannotCompileException("no inheritable constructor in "
					+ superclazz.getName());
