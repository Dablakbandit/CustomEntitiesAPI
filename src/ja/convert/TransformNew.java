...
final public class TransformNew extends Transformer {
	...
	public TransformNew(Transformer next, String classname, String trapClass,
			String trapMethod) {
		super(next);
	...
	public int transform(CtClass clazz, int pos, CodeIterator iterator,
		...
		if (c == NEW) {
			...
			if (cp.getClassInfo(index).equals(classname)) {
				...
				StackMapTable smt = (StackMapTable) iterator.get()
						.getAttribute(StackMapTable.tag);
				if (smt != null)
					smt.removeNew(pos);

				StackMap sm = (StackMap) iterator.get().getAttribute(
						StackMap.tag);
				if (sm != null)
					sm.removeNew(pos);
			}
	...
	private int computeMethodref(int typedesc, ConstPool cp) {
		...
		typedesc = cp.addUtf8Info(Descriptor.changeReturnType(classname,
