...
public abstract class CtBehavior extends CtMember {
	...
	/**
	 * Returns the method or constructor name followed by parameter types such
	 * as <code>ja.CtBehavior.stBody(String)</code>.
	 * 
	 * @since 3.5
	 */
	 ...
	 * object, you might have to explicitly rebuild a stack map table. ja does
	 * not automatically rebuild it for avoiding unnecessary rebuilding.
	 * 
	 ...
	 * <code>MethodInfo</code> might be also pruned.
	 * 
	 * @see #getMethodInfo()
	 ...
	 * 
	 * @return modifiers encoded with <code>ja.Modifier</code>.
	 * @see Modifier
	 */
	...
	public boolean hasAnnotation(Class clz) {
		...
		return CtClassType.hasAnnotationType(clz, getDeclaringClass()
	 ...
	 * this method/constructor, an <code>Author</code> object is returned. The
	 * member values can be obtained by calling methods on the
	...
	public Object getAnnotation(Class clz) throws ClassNotFoundException {
		...
		AnnotationsAttribute ainfo2 = (AnnotationsAttribute) mi
				.getAttribute(AnnotationsAttribute.visibleTag);
		return CtClassType.getAnnotationType(clz, getDeclaringClass()
				.getClassPool(), ainfo, ainfo2);
	}
	...
	private Object[] getAnnotations(boolean ignoreNotFound)
		...
		AnnotationsAttribute ainfo2 = (AnnotationsAttribute) mi
				.getAttribute(AnnotationsAttribute.visibleTag);
		return CtClassType.toAnnotationType(ignoreNotFound, getDeclaringClass()
				.getClassPool(), ainfo, ainfo2);
	}
	 ...
	 * descriptor, which is defined in the JVM specification. If two
	...
	/**
	 * Returns the generic signature of the method. It represents parameter
	 * types including type variables.
	 * 
	 * @see SignatureAttribute#toMethodSignature(String)
	 * @since 3.17
	 */
	public String getGenericSignature() {
		SignatureAttribute sa = (SignatureAttribute) methodInfo
				.getAttribute(SignatureAttribute.tag);
		return sa == null ? null : sa.getSignature();
	}

	/**
	 * Set the generic signature of the method. It represents parameter types
	 * including type variables. See
	 * {@link ja.CtClass#setGenericSignature(String)} for a code sample.
	 * 
	 * @param sig
	...
	public void setGenericSignature(String sig) {
		declaringClass.checkModify();
		methodInfo.addAttribute(new SignatureAttribute(methodInfo
				.getConstPool(), sig));
	}
	 ...
	 * 
	 * @param src
	 *            the source code representing the body. It must be a single
	 *            statement or block. If it is <code>null</code>, the
	 *            substituted body does nothing except returning zero or null.
	 */
	 ...
	 *            the name of the method that is called by
	 *            <code>$proceed()</code>.
	 */
	public void setBody(String src, String delegateObj, String delegateMethod)
		...
		try {
			...
			methodInfo.setAccessFlags(methodInfo.getAccessFlags()
					& ~AccessFlag.ABSTRACT);
			methodInfo
					.rebuildStackMapIf6(cc.getClassPool(), cc.getClassFile2());
	...
	static void setBody0(CtClass srcClass, MethodInfo srcInfo,
			CtClass destClass, MethodInfo destInfo, ClassMap map)
			throws CannotCompileException {
	 ...
	 * format. It is not an annotation. See {@link ja.bytecode.AttributeInfo}.
	 * 
	 * @param name
	 *            attribute name
	 */
	...
	/**
	 * Declares to use <code>$cflow</code> for this method/constructor. If
	 * <code>$cflow</code> is used, the class files modified with ja requires a
	 * support class <code>ja.runtime.Cflow</code> at runtime (other ja classes
	 ...
	 * indicated by <code>$cflow(Point.paint)</code>.
	 * 
	 * @param name
	...
	/**
	 * Declares a new local variable. The scope of this variable is the whole
	 ...
	 * visible from the code inserted by <code>insertAfter()</code>.
	 * 
	 * @param name
	...
	public void addLocalVariable(String name, CtClass type)
		...
		LocalVariableAttribute va = (LocalVariableAttribute) ca
		...
		va.addEntry(0, ca.getCodeLength(), cp.addUtf8Info(name),
				cp.addUtf8Info(desc), maxLocals);
	...
	private void addParameter2(int where, CtClass type, String desc)
		...
		if (ca != null) {
			...
			LocalVariableTypeAttribute lvta = (LocalVariableTypeAttribute) ca
					.getAttribute(LocalVariableTypeAttribute.tag);
			if (lvta != null)
				lvta.shiftIndex(where, size);

			StackMapTable smt = (StackMapTable) ca
					.getAttribute(StackMapTable.tag);
			if (smt != null)
				smt.insertLocal(where, StackMapTable.typeTagOf(typeDesc),
						classInfo);

			StackMap sm = (StackMap) ca.getAttribute(StackMap.tag);
			if (sm != null)
				sm.insertLocal(where, StackMapTable.typeTagOf(typeDesc),
						classInfo);
		}
	}
	 ...
	 * such as <code>insertBefore()</code> may collapse the bytecode because the
	 * <code>ExprEditor</code> loses its current position.
	 * 
	 * @param editor
	 *            specifies how to modify.
	 ...
	 * inserted bytecode is subject to constraints described in Section 4.8.2 of
	 * The Java Virtual Machine Specification (2nd ed). For example, it cannot
	 * access instance fields or methods although it may assign a value to an
	 * instance field directly declared in this class. Accessing static fields
	 * and methods is allowed. Use <code>insertBeforeBody()</code> in
	...
	/**
	 * Inserts bytecode at the end of the body. The bytecode is inserted just
	 * before every return insturction. It is not executed when an exception is
	 * thrown.
	 * 
	 * @param src
	 *            the source code representing the inserted bytecode. It must be
	...
	/**
	 * Inserts bytecode at the end of the body. The bytecode is inserted just
	 * before every return insturction.
	 * 
	 * @param src
	 *            the source code representing the inserted bytecode. It must be
	 *            a single statement or block.
	 * @param asFinally
	 *            true if the inserted bytecode is executed not only when the
	 *            control normally returns but also when an exception is thrown.
	 *            If this parameter is true, the inserted code cannot access
	 *            local variables.
	 */
	public void insertAfter(String src, boolean asFinally)
		...
		try {
			...
			int handlerLen = insertAfterHandler(asFinally, b, rtype, varNo, jv,
					src);
			int handlerPos = iterator.getCodeLength();
			if (asFinally)
				ca.getExceptionTable().add(getStartPosOfBody(ca), handlerPos,
						handlerPos, 0);
			...
			while (iterator.hasNext()) {
				...
				if (c == Opcode.ARETURN || c == Opcode.IRETURN
					...
					if (noReturn) {
						...
						adviceLen = insertAfterAdvice(b, jv, src, pool, rtype,
								varNo);
	...
	private int insertAfterAdvice(Bytecode code, Javac jv, String src,
			ConstPool cp, CtClass rtype, int varNo) throws CompileError {
	...
	private int insertAfterHandler(boolean asFinally, Bytecode b,
			CtClass rtype, int returnVarNo, Javac javac, String src)
			throws CompileError {
		if (!asFinally)
			return 0;
	 ...
	 * Modifier.isStatic(getModifiers())); CtClass rtype = getReturnType0(); int
	 ...
	 * Opcode.DRETURN || c == Opcode.RETURN) iterator.insert(pos, code); } }
	 * catch (NotFoundException e) { throw new CannotCompileException(e); }
	 * catch (CompileError e) { throw new CannotCompileException(e); } catch
	 * (BadBytecode e) { throw new CannotCompileException(e); } }
	 */

	/**
	 * Adds a catch clause that handles an exception thrown in the body. The
	 * catch clause must end with a return or throw statement.
	 * 
	 * @param src
	 *            the source code representing the catch clause. It must be a
	 ...
	 *            the type of the exception handled by the catch clause.
	 * @param exceptionName
	 *            the name of the variable containing the caught exception, for
	 *            example, <code>$e</code>.
	 */
	 ...
	 * 
	 * @param lineNum
	 *            the line number. The bytecode is inserted at the beginning of
	 ...
	 * specified. For example, if there is only a closing brace at that line,
	 * the bytecode would be inserted at another line below. To know exactly
	 * where the bytecode will be inserted, call with <code>modify</code> set to
	 * <code>false</code>.
	 * 
	 * @param lineNum
	 *            the line number. The bytecode is inserted at the beginning of
	 *            the code at the line specified by this line number.
	 * @param modify
	 *            if false, this method does not insert the bytecode. It instead
	 *            only returns the line number at which the bytecode would be
	 *            inserted.
	 * @param src
	 *            the source code representing the inserted bytecode. It must be
	 *            a single statement or block. If modify is false, the value of
	 *            src can be null.
	 * @return the line number at which the bytecode has been inserted.
	 */
	public int insertAt(int lineNum, boolean modify, String src)
		...
		try {
			...
			/*
			 * We assume that there is no values in the operand stack at the
			 * position where the bytecode is inserted.
			 */
			if (stack > ca.getMaxStack())
				ca.setMaxStack(stack);
