...
class CtClassType extends CtClass {
	...
	protected void extendToString(StringBuffer buffer) {
		...
		exToString(buffer, " fields=", memCache.fieldHead(),
				memCache.lastField());
		exToString(buffer, " constructors=", memCache.consHead(),
				memCache.lastCons());
		exToString(buffer, " methods=", memCache.methodHead(),
				memCache.lastMethod());
	}

	private void exToString(StringBuffer buffer, String msg, CtMember head,
			CtMember tail) {
		buffer.append(msg);
		while (head != tail) {
			head = head.next();
	...
	public String getGenericSignature() {
		SignatureAttribute sa = (SignatureAttribute) getClassFile2()
	...
	public void replaceClassName(ClassMap classnames) throws RuntimeException {
		...
		String newClassName = (String) classnames.get(Descriptor
				.toJvmName(oldClassName));
		...
		for (int i = 0; i < n; ++i)
			if (name.equals(ica.innerClass(i))) {
				...
				if (outName != null)
					...
					EnclosingMethodAttribute ema = (EnclosingMethodAttribute) cf
							.getAttribute(EnclosingMethodAttribute.tag);
					if (ema != null)
		...
		if (cc instanceof CtClassType) {
			...
			while (mth != mthTail) {
				...
				if (mth.getName().equals(name)
						&& ((CtMethod) mth).getMethodInfo2().getDescriptor()
								.equals(desc))
					return (CtMethod) mth;
			}
		}
		...
		if (init != null) {
			...
			if (Modifier.isStatic(mod) && Modifier.isFinal(mod))
				try {
					...
					if (index != 0) {
						f.getFieldInfo2().addAttribute(
								new ConstantAttribute(cp, index));
						init = null;
					}
				} catch (NotFoundException e) {
				}
	...
	private void modifyClassConstructor(ClassFile cf)
		...
		for (FieldInitLink fi = fieldInitializers; fi != null; fi = fi.next) {
			...
			if (Modifier.isStatic(f.getModifiers())) {
				doInit = true;
				int s = fi.init.compileIfStatic(f.getType(), f.getName(), code,
						jv);
				if (stacksize < s)
					stacksize = s;
			}
	...
	private void modifyClassConstructor(ClassFile cf, Bytecode code,
			int stacksize, int localsize) throws CannotCompileException {
			...
			if (minfo.isConstructor()) {
				...
				if (codeAttr != null)
					try {
						...
						CtClass[] params = Descriptor.getParameterTypes(
								minfo.getDescriptor(), classPool);
