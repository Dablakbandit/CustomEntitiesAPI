...
public class CustomEntityZombieHelper extends CustomEntityMonsterHelper {
	...
	public static void setGoalSelectorDefaultPathfinderGoals(Object zombie) {
		...
		try {
			...
			Field f = config1.getClass().getField(
					"zombieAggressiveTowardsVillager");
			Object value = f.get(config1);
			if ((boolean) value) {
				newGoalSelectorPathfinderGoalMeleeAttack(zombie,
						"EntityVillager", 1.0D, true);
			}
		} catch (Exception e) {
			newGoalSelectorPathfinderGoalMeleeAttack(zombie, "EntityVillager",
					1.0D, true);
		}
