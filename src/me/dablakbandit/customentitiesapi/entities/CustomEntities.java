...
public class CustomEntities {
	...
	private static void clone(Object oold, Object onew) {
		...
		try {
			...
			entity.getMethod("a", entity, boolean.class).invoke(
					entity.cast(onew), entity.cast(oold), true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		/*
		 * Map<Class<?>, Map<String, Object>> fields = new HashMap<Class<?>,
		 * Map<String, Object>>(); Class<?> c = oold.getClass(); for(int i = 0;
		 * i<6; i++){ Map<String, Object> fs1 = new HashMap<String, Object>();
		 * for(Field f : c.getDeclaredFields()){ f.setAccessible(true);
		 * if(!Modifier.isFinal(f.getModifiers())&&!f.getName().equals("dead")){
		 * try { fs1.put(f.getName(), f.get(oold)); } catch (Exception e) {
		 * e.printStackTrace(); } } } fields.put(c, fs1); c = c.getSuperclass();
		 * } for(Class<?> c1 : fields.keySet()){ Map<String, Object> fs
		 * =fields.get(c1); for(String f : fs.keySet()){ try { try{ Field f1 =
		 * c1.getDeclaredField(f); f1.setAccessible(true); f1.set(c1.cast(onew),
		 * fs.get(f)); }catch(Exception e1){
		 * 
		 * } } catch (Exception e) { e.printStackTrace(); } } } Field[] fi =
		 * c.getDeclaredFields(); for(int i = 0; i < fi.length; i++){ Field f =
		 * fi[i]; f.setAccessible(true); }
		 */
	}
	...
	public static CustomEntityIronGolem getCustomEntityIronGolem(Entity e) {
		if (isCustomEntityIronGolem(e)) {
			...
			CustomEntityIronGolem ceig = new CustomEntityIronGolem(
					e.getLocation());
	 ...
	 * @see CustomEntityIronGolem
	 */
	public static CustomEntityIronGolem getNewCustomEntityIronGolem(
			Location location) {
	 ...
	 * @see CustomEntitySkeleton
	 */
	public static CustomEntitySkeleton getNewCustomEntitySkeleton(
			Location location) {
	 ...
	 * @see CustomEntityVillager
	 */
	public static CustomEntityVillager getNewCustomEntityVillager(
			Location location) {
	...
	public static boolean isCustomEntityIronGolem(Entity e) {
		return NMSUtils.getHandle(e).getClass().getName()
				.equals("temp.CustomEntityIronGolem");
	}
	...
	public static boolean isCustomEntitySkeleton(Entity e) {
		return NMSUtils.getHandle(e).getClass().getName()
				.equals("temp.CustomEntitySkeleton");
	}
	...
	public static boolean isCustomEntityVillager(Entity e) {
		return NMSUtils.getHandle(e).getClass().getName()
				.equals("temp.CustomEntityVillager");
	}
	...
	public static boolean isCustomEntityZombie(Entity e) {
		return NMSUtils.getHandle(e).getClass().getName()
				.equals("temp.CustomEntityZombie");
	}
	...
	public static boolean isNormalEntityIronGolem(Entity e) {
		return NMSUtils
				...
				.getClass()
				.getName()
				.equals("net.minecraft.server." + NMSUtils.getVersion()
						+ "EntityIronGolem");
	}
